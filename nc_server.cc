...
int Connections::OpenConnection(const struct connection *conn)
{
    ...
    for (iconn = 0; iconn < _connections.size(); iconn++)
        ...
        if (!_connections[iconn])
            break;

    if (iconn == _connections.size())
        _connections.push_back(cp);
    else
        _connections[iconn] = cp;
...
int Connections::CloseOldConnections()
{
    ...
    for (i = 0; i < _connections.size(); i++)
        if (_connections[i]
            && (utime - _connections[i]->LastRequest() >
                CONNECTIONTIMEOUT)) {
...
{
    unsigned int i, n;
    for (i = n = 0; i < _connections.size(); i++)
        if (_connections[i])
            n++;
    return n;
}
...
Connection *&Connections::operator[] (unsigned int i)
{
#ifdef DEBUG
...
FileGroup::FileGroup(const struct connection *conn):_interval(conn->
                                                              interval),
...
{
    close();
    for (unsigned int i = 0; i < _vargroups.size(); i++)
        delete _vargroups[i];
}
...
int FileGroup::check_file(const string & fileName) const
{
    ...
    try {
        ...
        nidas::util::Process nccheck =
            nidas::util::Process::spawn("nccheck", args);
        int status;
        string errmsg;
...
int FileGroup::add_var_group(const struct datadef *dd)
{
    ...
    for (i = 0; i < n; i++)
        if (_vargroups[i]->same_var_group(dd))
            return i;
...
NS_NcFile *FileGroup::put_rec(const datarec_float * writerec,
                              NS_NcFile * f)
{
    ...
    if (!f->put_rec(writerec, _vargroups[ngroup], dtime))
        f = 0;
    return f;
}

NS_NcFile *FileGroup::put_rec(const datarec_int * writerec, NS_NcFile * f)
{
    int ngroup = writerec->datarecId;
    double dtime = writerec->time;
    ...
    for (i = 0; i < nv; i++) {
        ...
        if (ov->isCnts()) {
            ...
            for (j = 0; j < nv; j++) {
                if (j != i) {
                    ov2 = _outvars[j];
                    // compare against original (Variable) name of counts var
                    if ((cntsattr = ov2->att_val("counts")).length() > 0 &&
                        cntsattr == v1->name())
...
Variable::Variable(const Variable & v):_name(v._name),_isCnts(v._isCnts)
{
    copy_atts(v);
}
...
void Variable::add_att(const string& name, const string& val)
{
    ...
    if (val.length() > 0) {
        if (mi == _strAttrs.end()) _attrNames.push_back(name);
        _strAttrs[name] = val;
    }
    else {
        if (mi != _strAttrs.end()) {
            vector<string>::iterator vi =
                std::find(_attrNames.begin(),_attrNames.end(),name);
            if (vi != _attrNames.end()) _attrNames.erase(vi);
            _strAttrs.erase(mi);
        }
    }
}
...
void Variable::copy_atts(const Variable& v)
{
    _strAttrs = v._strAttrs;
    _attrNames = v._attrNames;
}
     ...
     _dimNames(0),_dimSizes(0),_dimIndices(0),_dims(0),_ndimalloc(0)
{
    ...
    if (_ttType == FIXED_DELTAT) {
        ...
        if (!(intervalAtt = _timeOffsetVar->get_att("interval(sec)"))) {
            char val[16];
            sprintf(val,"%f",_interval);
            if (!_timeOffsetVar->add_att("interval(sec)",val)) {
                PLOG(("add_att %s: %s %s %s", _fileName.c_str(),
            ...
            if ((cntsAttr = ov->att_val("counts")).length() > 0) {
                if (countsAttrNameFromOVs.length() > 0) {
                    if (countsAttrNameFromOVs != cntsAttr)
                        PLOG(("multiple counts attributes for group with %s: %s and %s",
                            ov->name().c_str(), countsAttrNameFromOVs.c_str(), cntsAttr.c_str()));
                } else
                    ...
                    if (att->type() == ncChar && att->num_vals() > 0) {
                        ...
                        if (countsAttrNameFromFile.length() == 0)
                            ...
                            if (countsAttrNameFromFile != string(cname))
                                PLOG(("multiple counts variables for group with %s: %s and %s",
                                    ov->name().c_str(), countsAttrNameFromFile.c_str(), cname));
                        }
                        delete [] cname;
                    }
                    delete att;
...
NS_NcVar *NS_NcFile::add_var(OutVariable * v)
{
    ...
    if (!(var = find_var(v))) {
        ...
        if (shortName.length() > 0 && !var->add_att("short_name", shortName.c_str())) {
            DLOG(("add_att %s: %s %s %s", _fileName.c_str(), varName.c_str(),
                  "short_name", nc_strerror(ncerror.get_err())));
            goto error;
        }
    }
    ...
    for (i = 0; i < var->num_dims(); i++) {
        ...
        DLOG(("dim name=%s,size=%d", dim->name(), dim->size()));
    }
#endif
    // double check ourselves
    if (!check_var_dims(var)) {
        PLOG(("wrong dimensions for variable %s: %s", _fileName.c_str(),
              varName.c_str()));
        goto error;
    }
...
int NS_NcFile::add_attrs(OutVariable * v, NcVar * var)
{
    ...
    for (unsigned int i = 0; i < attrNames.size(); i++) {
        ...
        if (aval.length() > 0) {
            if (!(nca = var->get_att(aname.c_str()))) {
                if (!var->add_att(aname.c_str(), aval.c_str())) {
                    PLOG(("add_att %s: %s %s %s", _fileName.c_str(),
                          var->name(), aname.c_str(),
                          nc_strerror(ncerror.get_err())));
                    return -1;
                }
            } else
                          ...
                          VariableGroup * vgroup, double dtime)
{
    ...
    if (_ttType == FIXED_DELTAT && vgroup->num_samples() > 1) {
        ...
        if (_timesAreMidpoints) tdiff = nsample * groupInt;
        else tdiff = (nsample + .5) * groupInt - .5 * _interval;
...
NcBool NS_NcFile::put_rec(const struct datarec_int * writerec,
                          VariableGroup * vgroup, double dtime)
{
    long nrec;
    long nsample;
    ...
    long *start = (long *) writerec->start.start_val;
    long *count = (long *) writerec->count.count_val;
    const int *d = (const int *) writerec->data.data_val;
    int nd = writerec->data.data_len;
    const int *dend = (const int *) d + nd;
    ...
    if (d != dend)
        PLOG(("put check %s: %s put request for %d values, should be %d",
              _fileName.c_str(), vars[0]->name(), nd,
              d - (const int *) writerec->data.data_val));
...
long NS_NcFile::put_time(double timeoffset, const char *varname)
{
    ...
    if (_ttType == VARIABLE_DELTAT)
        ...
        if (_timesAreMidpoints)
            nrec = (long) (timeoffset / _interval);
        else
...
#ifdef DEBUG
    DLOG(("after fill timeoffset = %f, timeOffset=%f,nrec=%d, _nrecs=%d,interval=%f", (double) timeoffset, (double) _timeOffset, nrec, _nrecs, _interval));
#endif
