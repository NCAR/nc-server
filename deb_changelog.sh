...
/^nc-server/ { print $0 " (" version ") stable; urgency=low" }
...
git log --max-count=100 --date-order --format="%H%nnc-server%n  * (XXXXXXX) %s%n -- %aN <%ae>  %cD" --date=local ${sincetag}.. | awk -f $awkcom
